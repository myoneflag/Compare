...
MathJax.Hub.Register.StartupHook("TeX Jax Ready", function() {
    ...
    TEX.Parse.Augment({
        ...
        HandleTag: function(name) {
            ...
            global.tag = MML.mtd.apply(MML, this.InternalMath(arg)).With({
                id: CONFIG.formatID(tag)
            });
        },
        ...
        HandleLabel: function(name) {
            ...
            if (!AMS.refUpdate) {
                ...
                AMS.eqlabels[label] = {
                    tag: "???",
                    id: ""
                }; // will be replaced by tag value later
            }
        },
        ...
        Genfrac: function(name, left, right, thick, style) {
            ...
            if (left || right) {
                frac = TEX.fixedFence(left, frac.With({
                    texWithDelims: true
                }), right)
            }
            if (style !== "") {
        ...
        xArrow: function(name, chr, l, r) {
            ...
            mml.SetData(mml.over, MML.mpadded(top).With(def).With({
                voffset: ".15em"
            }));
            if (bot) {
                bot = TEX.Parse(bot, this.stack.env).mml()
                mml.SetData(mml.under, MML.mpadded(bot).With(def).With({
                    voffset: "-.24em"
                }));
            }
            this.Push(mml.With({
