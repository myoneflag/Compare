...
(function(TEX, HUB, AJAX) {
    ...
    var STACKITEM = STACK.Item = MathJax.Object.Subclass({
        ...
        mmlData: function(inferred, forceRow) {
            ...
            return MML.mrow.apply(MML, this.data).With((inferred ? {
                inferred: true
            } : {}));
        },
            ...
            if (item.type === "mml") {
                if (this.primes) {
                    if (this.position !== 2) {
                        ...
                        item.data[0] = MML.mrow(this.primes.With({
                            variantForm: true
                        }), item.data[0])
                    }
                }
    ...
    STACKITEM.fn = STACKITEM.Subclass({
        ...
        checkItem: function(item) {
            if (this.data[0]) {
                ...
                return [this.data[0], MML.mo(MML.entity("#x2061")).With({
    ...
    STACKITEM.not = STACKITEM.Subclass({
        ...
        checkItem: function(item) {
            ...
            //  \mathrel{\rlap{\notChar}}
            mml = MML.mpadded(MML.mtext("\u29F8")).With({
                width: 0
            });
    ...
    var PARSE = MathJax.Object.Subclass({
        ...
        csMathchar0mi: function(name, mchar) {
            var def = {
                mathvariant: MML.VARIANT.ITALIC
            };
            if (isArray(mchar)) {
                def = mchar[1];
                mchar = mchar[0]
        ...
        Color: function(name) {
            ...
            this.Push(MML.mstyle(math).With({
                mathcolor: color
            }));
        },
        ...
        LeftRight: function(name) {
            this.Push(STACKITEM[name.substr(1)]().With({
        ...
        Accent: function(name, accent, stretchy) {
            ...
            this.Push(MML.TeXAtom(MML.munderover(c, null, mml).With({
                accent: true
            })));
        },
        ...
        UnderOver: function(name, c, stack, noaccent) {
            ...
            if (base.isa(MML.munderover) && base.isEmbellished()) {
                base.Core().With({
                    lspace: 0,
                    rspace: 0
                }); // get spacing right for NativeMML
                base = MML.mrow(MML.mo().With({
                    rspace: 0
                }), base); // add an empty <mi> so it's not embellished any more
            }
        ...
        Lap: function(name) {
            var mml = MML.mpadded(this.ParseArg(name)).With({
                width: 0
            });
            if (name === "\\llap") {
                mml.lspace = "-1width"
        ...
        BuildRel: function(name) {
            ...
            var bot = this.ParseArg(name);
            this.Push(MML.TeXAtom(MML.munderover(bot, null, top)).With({
                texClass: MML.TEXCLASS.REL
            }));
        },
        ...
        FBox: function(name) {
            this.Push(MML.menclose.apply(MML, this.InternalMath(this.GetArgument(name))).With({
                notation: "box"
            }));
        },
        ...
        Dots: function(name) {
            this.Push(STACKITEM.dots().With({
                ldots: this.mmlToken(MML.mo(MML.entity("#x2026")).With({
                    stretchy: false
                })),
                cdots: this.mmlToken(MML.mo(MML.entity("#x22EF")).With({
                    stretchy: false
                }))
            }));
        },
        ...
        CrLaTeX: function(name) {
            ...
            if (top.isa(STACKITEM.array)) {
                ...
                this.Push(MML.mspace().With({
                    linebreak: MML.LINEBREAK.NEWLINE
                }));
            }
        },
        emPerInch: 7.2,
    ...
    TEX.Augment({
        ...
        Error: function(message) {
            ...
            throw HUB.Insert(Error(message), {
                texError: true
            });
        },
